...
def create_sentiment_distribution_chart(sentiment_data):
    ...
    # Create data for pie chart with percentages in labels
    values = [overall['positive'], overall['neutral'], overall['negative']]
    total = sum(values)
    
    # Format percentages for labels
    percent_positive = overall['positive'] if isinstance(overall['positive'], (int, float)) else 0
    percent_neutral = overall['neutral'] if isinstance(overall['neutral'], (int, float)) else 0
    percent_negative = overall['negative'] if isinstance(overall['negative'], (int, float)) else 0
    
    labels = [f'Positive ({percent_positive:.1f}%)', 
              f'Neutral ({percent_neutral:.1f}%)', 
              f'Negative ({percent_negative:.1f}%)']
    ...
    # Add counts to hover information
    fig.update_traces(
        hoverinfo='label+percent+value',
        textfont_size=14
    )
    
    # Format title to show sentence-based analysis
    total_sentences = int(overall['total_reviews']) if isinstance(overall['total_reviews'], (int, float)) else 0
    fig.update_layout(
        title=f"Review Sentiment Distribution (Based on {total_sentences} sentences)",
        height=350,
        legend=dict(
            orientation="h",
            yanchor="bottom",
